...
def fetch_landsat_ndvi(bounds, date_str, out_size=512, max_cloud_cover=20,
    ...
    `bounds`/`out_size`, and compute cloud-masked NDVI.

    Returns a (out_size, out_size) float32 NDVI array with cloud/shadow
    pixels set to NaN, or None if no usable real scene was found.
    """
    ...
    try:
        red = _read_band_clip(item.assets['red'].href, bounds, out_size)
        nir = _read_band_clip(item.assets['nir08'].href, bounds, out_size)
        qa = _read_band_clip(
            item.assets['qa_pixel'].href, bounds, out_size, resampling=Resampling.nearest
        )
...
def _read_band_clip(href, bounds, out_size, resampling=Resampling.bilinear):
    """
    Read a single band from a remote COG, reprojected onto a common
    EPSG:4326 grid matching `bounds`/`out_size`.
    ...
    explicitly requested in EPSG:4326), so NDVI delta math between two
    dates -- even across different sensors -- compares the same ground
    locations.
    """
    dst_transform = transform_from_bounds(*bounds, out_size, out_size)
